...
class TCamera {
   ...
   public:
    TCamera(size_t row, size_t col, double focalLength)
        : _row(row), _col(col), _picture(row, col) {
        ...
        for (size_t i = 0; i < _row; ++i) {
            _offsetRow.push_back(+height / 2. -
                                 (i + 0.5) * height / double(_row));
        }
        for (size_t i = 0; i < _col; ++i) {
            _offsetCol.push_back(+width / 2. -
                                 (i + 0.5) * width / double(_col));
        }
    };

    TRay castRay(size_t row, size_t col) const {
        return TRay(TPoint(0., 0., 0.),
                    TVector(1., _offsetRow[row], _offsetCol[col]));
    }

    TRay castRay(size_t row, size_t col, double subRow, double subCol) const {
        return TRay(TPoint(0., 0., 0.),
                    TVector(1., _offsetRow[row] + _widthPerPixel * subRow,
